...
## Документация — обязательно после задач

После завершения значимой задачи (commit, миграция, деплой, новая фича, фикс бага) — **сразу** обнови соответствующую документацию:

- **`docs/FEATURES_STATUS.md`** — реестр функций. Помечай ✅/🟡/🔲, добавляй новые в нужную секцию, отмечай в каком v# реализовано.
- **`docs/HANDOFF.md`** — контекст для следующей сессии: текущая версия, доступы, что задеплоено, что в работе.
- **`docs/TROUBLESHOOTING.md`** — каждая встреченная ошибка + фикс. Симптом / причина / решение.

Не накапливать задачи на конец сессии — обновлять документацию параллельно с кодом, в одном коммите.

Перед `git commit` проверять: «обновил ли я документацию по этому изменению?». Если нет — добавить в тот же коммит.
