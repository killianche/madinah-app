...
export default async function TeacherHome({
  ...
  // Next-up слоты: все запланированные сегодня, у которых время ещё не прошло на 30+ мин.
  // Для других дат (просмотр истории/будущего) блок не показываем — см. ниже isToday.
  const nowMinutes = (() => {
    const n = new Date();
    return n.getHours() * 60 + n.getMinutes();
  })();
  const heroUpcomingToday = scheduled.filter((s) => {
    ...
    <AppShell>
      ...
      {totalPlanned > 0 && (
        ...
        >
          ...
          {/* Next-up: все ученики у кого сегодня урок и время не прошло > 30 мин */}
              ...
              {heroUpcomingToday.map((s, i) => (
                <div
                  ...
                  className={`flex items-center gap-3 ${
                    i > 0 ? "pt-2 mt-2 border-t border-border-cream" : ""
                  }`}
                >
                  <span
                    className="w-2 h-2 rounded-full bg-terracotta shrink-0"
                    style={i === 0 ? { boxShadow: "0 0 0 3px rgba(201,100,66,0.18)" } : undefined}
                  />
                  ...
                  {s.slot_time && (
                    <span className="font-serif text-[20px] font-medium tabular-nums">
                      {s.slot_time}
                    </span>
                  )}
                </div>
