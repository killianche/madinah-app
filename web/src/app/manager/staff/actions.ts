...
> {
  ...
  try {
    // Если создаём учителя без привязки к существующей записи teachers —
    // создаём её сами, иначе у учителя не будет профиля (страница «Сегодня»
    // ищет teachers по user_id и без этой строки падает с «профиль не настроен»).
    let teacherId = parsed.data.teacher_id || null;
    if (parsed.data.role === "teacher" && !teacherId) {
